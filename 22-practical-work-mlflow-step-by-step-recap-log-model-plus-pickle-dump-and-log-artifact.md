...
# Chapter 22 — Step-by-step recap: combining `mlflow.sklearn.log_model` with a manual `pickle.dump` and `mlflow.log_artifact("elastic-net-regression.pkl")`
...
| 2 | [What we add today vs chap 21](#section-2) |
...
## 2. What we add today vs chap 21
...
| Drop `registered_model_name` (chap 21's lesson) | Today's focus is the artifact pattern. |
...
```text
chap22-mlflow-step-by-step-recap-log-model-plus-pickle-dump-and-log-artifact/
├── README.md
├── docker-compose.yml
├── data/
...
```bash
cd chap22-mlflow-step-by-step-recap-log-model-plus-pickle-dump-and-log-artifact
...
This closes the **01 -> v** recap series. Coverage map:

| Block | Chapters | Theme |
|---|---|---|
| Setup | 01, 02 | `set_tracking_uri`, `set_experiment`, `start_run`, basic params/metrics |
| Pipeline | 03 | Full ElasticNet on red wine quality |
| Docker plumbing | 04, 05 | `MLFLOW_TRACKING_URI` env var, debugging the connection |
| Experiments & runs | 06, 07, 09, 10, 11 | `create_experiment`, `active_run`, `set_tags`, multiple runs/experiments |
| Artifacts & bulk | 08 | `log_artifacts(folder)`, `log_params/metrics(dict)` |
| Autolog | 12 | `mlflow.sklearn.autolog` |
| Storage | 13 | PostgreSQL backend store + S3 artifact storage |
| Signatures | 14, 15 | `infer_signature` and manual `Schema/ColSpec` |
| Pyfunc + custom envs | 16 | `pyfunc.log_model` with wrapper, `joblib`, `conda_env` |
| Loading & evaluating | 17, 18, 19 | `pyfunc.load_model`, `mlflow.evaluate` (default + custom) |
| Decision gates | 20 | `validation_thresholds` + `baseline_model` |
| Registry | 21 | `registered_model_name="elasticnet-api"` |
| Dual-format | 22 | `log_model` + `pickle.dump` + `log_artifact` |

Next: **[Chapter 23](./23-practical-work-mlflow-step-by-step-recap-mlflow-register-model-post-hoc-and-load-by-version.md)** — register an existing run's model **after the fact** with `mlflow.register_model(model_uri, name)` (so you don't have to decide at training time), then load it back deterministically by version with `mlflow.pyfunc.load_model(f"models:/<name>/<version>")`.
...
<p align="center">
  <strong>End of Chapter 22 — log_model + pickle.dump + log_artifact</strong><br/>
