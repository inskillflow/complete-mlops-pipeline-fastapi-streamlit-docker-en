# chap09 - Step-by-step recap: attaching searchable metadata to runs with `set_tags`

The full lesson lives at [../09-practical-work-mlflow-step-by-step-recap-attaching-metadata-to-runs-with-set-tags.md](../09-practical-work-mlflow-step-by-step-recap-attaching-metadata-to-runs-with-set-tags.md).

> **In one line.** This chapter is about how to **attach free-form key/value metadata to a run (`set_tag`, `set_tags`) so you can filter runs by team, branch, dataset, framework version, etc. -- the lowest-friction way to keep MLflow organized as the repo grows**.
...
## What is new vs chap08

- `mlflow.set_tag(key, value)` -> attach a single tag to the active run
- `mlflow.set_tags({...})` -> bulk version
- Standard tag prefixes (`mlflow.source.*`, `mlflow.user`) -> what MLflow already sets for you automatically
- Filter runs in the UI with `tags.team = "ml-platform"` and `tags.framework = "sklearn"`
...
```bash
cd chap09-mlflow-step-by-step-recap...
```
...
```bash
docker compose ps
# mlflow-recap-09    Up X seconds (healthy)
```
...
```bash
docker compose run --rm trainer --alpha 0.3 --l1_ratio 0.7
```
...
- 1 run with the tags visible in the **Tags** section of the run page.

> **Chapter quirk.** Tags are run-level by default. To tag an **experiment** (so the tag follows every run), use `MlflowClient().set_experiment_tag(experiment_id, key, value)` -- a different API.
...
3. The MLflow service is healthy: `docker compose ps` -> `mlflow-recap-09 ... healthy`.
...
**Next**: [chap10](../10-practical-work-mlflow-step-by-step-recap-multiple-runs-in-one-experiment.md) -- factor the per-run logic into a helper `train_one_run(...)` and loop over a list of `CONFIGS` to create **multiple runs in one experiment**, ready for side-by-side comparison.