...
# Chapter 21 — Step-by-step recap: registering a model with `mlflow.sklearn.log_model(..., registered_model_name="elasticnet-api")`
...
| 2 | [What we add today vs chap 20](#section-2) |
...
Up to chap 20 every model lived inside the run that produced it. To consume one from another service (FastAPI, a batch scorer…) we had to know its `runs:/<run_id>/<artifact_path>` URI. That works, but:
...
## 2. What we add today vs chap 20

chap 20 was the climax of the evaluation series. To put the registry in the spotlight we **trim the script back** to a small, focused training script:

| Diff | What |
|---|---|
| Drop `pyfunc` wrapper, baseline, validation thresholds, custom metrics, scatter | All proven in 16–20. Today's lesson is the registry. |
...
```text
chap21-mlflow-step-by-step-recap-registered-model-name-with-mlflow-sklearn-log-model/
├── README.md
├── docker-compose.yml
├── data/
...
Standard MLflow + trainer (SQLite backend, identical to chap 17–19):
...
```bash
cd chap21-mlflow-step-by-step-recap-registered-model-name-with-mlflow-sklearn-log-model
...
<p align="center">
  <strong>End of Chapter 21 — registering a model with `registered_model_name`</strong><br/>
