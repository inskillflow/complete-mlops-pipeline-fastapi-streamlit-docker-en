...
# Chapter 26 — Step-by-step recap: packaging training as an `MLproject` and launching it with `mlflow.projects.run(...)`
...
| 2 | [What we add today vs chap 25](#section-2) |
...
## 2. What we add today vs chap 25
...
| `train.py` itself is **unchanged** from chap 25 | The packaging is purely additive. |
...
```yaml
name: chap26-elasticnet-project
...
```text
chap26-mlflow-step-by-step-recap-mlflow-projects-run-with-mlproject-yaml-and-entry-points/
├── README.md
├── docker-compose.yml
├── data/
...
└── trainer/
    ...
    ├── train.py               ← unchanged from chap 25
...
### 8.3 `trainer/train.py` (unchanged from chap 25)
...
Identical to chap 25. The trainer service mounts `./data` and points at the MLflow tracking server via `MLFLOW_TRACKING_URI`.
...
```bash
cd chap26-mlflow-step-by-step-recap-mlflow-projects-run-with-mlproject-yaml-and-entry-points
...
<p align="center">
  <strong>End of Chapter 26 — mlflow.projects.run with MLproject YAML and entry points</strong><br/>
