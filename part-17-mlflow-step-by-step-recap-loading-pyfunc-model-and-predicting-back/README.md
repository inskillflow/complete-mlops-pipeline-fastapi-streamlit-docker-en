# chap17 - Step-by-step recap: loading a pyfunc model and predicting back (`runs:/<run_id>/<artifact_path>`)

The full lesson lives at [../17-practical-work-mlflow-step-by-step-recap-loading-pyfunc-model-and-predicting-back.md](../17-practical-work-mlflow-step-by-step-recap-loading-pyfunc-model-and-predicting-back.md).

> **In one line.** This chapter is about how to **close the train -> log -> reload loop: pull the pyfunc you logged in chap16 back with `mlflow.pyfunc.load_model(f"runs:/{run_id}/sklearn_mlflow_pyfunc")` and run `.predict()` on the test split, comparing to the saved metrics**.
...
## What is new vs chap16

- `runs:/<run_id>/<artifact_path>` URI scheme -> MLflow's canonical "fetch artifact by run" pattern
- `loaded = mlflow.pyfunc.load_model(model_uri)` -> a `PyFuncModel` instance with a `.predict(pandas_df)` method
- Round-trip sanity check: `np.allclose(loaded.predict(X), retrained.predict(X))`
- Print the loaded `MLmodel` (`loaded.metadata`) to inspect signature / conda env / code path at load time
...
```bash
cd chap17-mlflow-step-by-step-recap...
```
...
```bash
docker compose ps
# mlflow-recap-17    Up X seconds (healthy)
```
...
- Experiment: `experiment_pyfunc_load`
- 1 run + console output showing the **freshly loaded** model's predictions matching the original RMSE/MAE/R2 to many decimal places.

> **Chapter quirk.** `models:/<name>/<version>` (registry URI) and `runs:/<run_id>/<artifact_path>` (run URI) **both** work with `pyfunc.load_model`. The registry URI is the production choice; the run URI is the dev/debug one.
...
# Open http://localhost:5000 and inspect the runs in experiment 'experiment_pyfunc_load'.
...
3. The MLflow service is healthy: `docker compose ps` -> `mlflow-recap-17 ... healthy`.
...
**Next**: [chap18](../18-practical-work-mlflow-step-by-step-recap-mlflow-evaluate-default-regressor.md) -- instead of computing RMSE/MAE/R2 by hand after `load_model`, let MLflow do it (and a lot more) with **`mlflow.evaluate(model_uri, eval_df, targets=..., model_type="regressor")`**.