...
# Chapter 23 — Step-by-step recap: registering with `mlflow.register_model(model_uri, name)` (post-hoc) and loading by `models:/<name>/<version>`
...
| 2 | [What we add today vs chap 22](#section-2) |
...
In chap 21 we registered a model in **one step** by passing `registered_model_name="elasticnet-api"` to `mlflow.sklearn.log_model(...)`. Chap 22 then logged the same model in **two formats** (MLflow-canonical + a side-car `pickle`). That `registered_model_name=` kwarg is convenient but fuses two distinct actions: **logging the artifact** and **adding a registry entry**.
...
## 2. What we add today vs chap 22
...
| Aspect | `registered_model_name=` kwarg (chap 21) | `mlflow.register_model(...)` (today) |
|---|---|---|
...
```text
chap23-mlflow-step-by-step-recap-mlflow-register-model-post-hoc-and-load-by-version/
├── README.md
├── docker-compose.yml
├── data/
...
Identical to chap 21 (SQLite backend is required for the registry).
...
```bash
cd chap23-mlflow-step-by-step-recap-mlflow-register-model-post-hoc-and-load-by-version
...
Next: **[Chapter 24](./24-practical-work-mlflow-step-by-step-recap-registering-a-model-trained-outside-mlflow-with-cloudpickle.md)** — register a model that was trained **completely outside** MLflow: load a `.pkl` produced by some other process, then push it into the registry with `mlflow.sklearn.log_model(loaded_model, ..., serialization_format="cloudpickle", registered_model_name=...)`.
...
<p align="center">
  <strong>End of Chapter 23 — mlflow.register_model post-hoc and load by version</strong><br/>
