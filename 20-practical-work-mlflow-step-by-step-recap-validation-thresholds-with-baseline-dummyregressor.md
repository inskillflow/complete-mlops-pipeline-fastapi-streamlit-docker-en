...
# Chapter 20 — Step-by-step recap: `mlflow.evaluate` with `validation_thresholds` (`MetricThreshold`) and a `baseline_model` (`DummyRegressor`)
...
| 2 | [What we add today vs chap 19](#section-2) |
...
In chap 19 we extended `mlflow.evaluate` with **custom metrics** and a **scatter artifact**. Today we add the **decision-gate** features:
...
## 2. What we add today vs chap 19
...
Everything else (`autolog`, `extra_metrics`, `custom_artifacts`, `set_tags`) is identical to chap 19.
...
In chap 16 the wrapper hardcoded `context.artifacts["sklearn_model"]`. Today we log **two** models (candidate + baseline), so each needs its own artifact key. We make the artifact key a constructor argument:
...
```text
chap20-mlflow-step-by-step-recap-validation-thresholds-with-baseline-dummyregressor/
├── README.md
├── docker-compose.yml
├── data/
...
# ===== custom metrics + artifact (same as chap 19) =====
...
Identical to chap 19 (matplotlib + joblib + cloudpickle + mlflow).
...
```bash
cd chap20-mlflow-step-by-step-recap-validation-thresholds-with-baseline-dummyregressor
...
<p align="center">
  <strong>End of Chapter 20 — validation thresholds with a baseline DummyRegressor</strong><br/>
