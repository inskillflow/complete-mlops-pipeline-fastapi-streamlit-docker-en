# chap20 - Step-by-step recap: validation thresholds + `baseline_model` (DummyRegressor) as a release gate

The full lesson lives at [../20-practical-work-mlflow-step-by-step-recap-validation-thresholds-with-baseline-dummyregressor.md](../20-practical-work-mlflow-step-by-step-recap-validation-thresholds-with-baseline-dummyregressor.md).

> **In one line.** This chapter is about how to **turn `mlflow.evaluate(...)` from a metrics logger into a **release gate**: declare `MetricThreshold(threshold=..., min_absolute_change=..., min_relative_change=..., higher_is_better=...)` and a `baseline_model` (a `DummyRegressor`) so a bad candidate raises `ModelValidationFailedException`**.
...
## What is new vs chap19

- `thresholds = {"r2_score": MetricThreshold(threshold=0.1, higher_is_better=True), "mean_squared_error": MetricThreshold(threshold=0.7, min_absolute_change=0.05, higher_is_better=False)}`
- `baseline = DummyRegressor(strategy="mean").fit(X_train, y_train)` -> the "must-beat" floor
- `mlflow.evaluate(..., validation_thresholds=thresholds, baseline_model=baseline_model_uri)`
- On failure: `ModelValidationFailedException` -> the process exits non-zero, perfect for CI/CD gates
...
```bash
cd chap20-mlflow-step-by-step-recap...
```
...
```bash
docker compose ps
# mlflow-recap-20    Up X seconds (healthy)
```
...
- 1 run if thresholds pass; otherwise the trainer exits with `ModelValidationFailedException`. Inspect the run -> Artifacts -> `eval_results_table.json` for the per-metric pass/fail diagnosis.

> **Chapter quirk.** `MetricThreshold` semantics combine: `threshold` is the absolute floor/ceiling, `min_absolute_change` / `min_relative_change` are deltas vs the baseline. All three checks run; ALL must pass. Read the failing exception carefully -- it lists every offending metric.
...
# Open http://localhost:5000 and inspect the runs in experiment 'experiment_validation'.
...
3. The MLflow service is healthy: `docker compose ps` -> `mlflow-recap-20 ... healthy`.
...
**Next**: [chap21](../21-practical-work-mlflow-step-by-step-recap-registered-model-name-with-mlflow-sklearn-log-model.md) -- register the trusted model in the **MLflow Model Registry** with `mlflow.sklearn.log_model(..., registered_model_name="elasticnet-api")` so downstream services can load it by alias.