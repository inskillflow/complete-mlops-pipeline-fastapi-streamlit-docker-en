# chap23 - Step-by-step recap: `mlflow.register_model(...)` post-hoc + load by version (`models:/<name>/<version>`)

The full lesson lives at [../23-practical-work-mlflow-step-by-step-recap-mlflow-register-model-post-hoc-and-load-by-version.md](../23-practical-work-mlflow-step-by-step-recap-mlflow-register-model-post-hoc-and-load-by-version.md).

> **In one line.** This chapter is about how to **register an already-logged model **after the fact** with `mlflow.register_model(model_uri="runs:/<id>/model", name="elastic-api-2")` -- the production pattern where training and registration are separate jobs -- then load it back via `models:/elastic-api-2/<version>`**.
...
## What is new vs chap22

- Drop `registered_model_name=` from `log_model(...)` -> logging stays pure logging
- `run = mlflow.active_run()` to capture the run id
- `mv = mlflow.register_model(model_uri=f"runs:/{run.info.run_id}/model", name="elastic-api-2")` -> returns a `ModelVersion` with `.version`
- Round-trip: `loaded = mlflow.pyfunc.load_model(f"models:/elastic-api-2/{mv.version}")` -> predict -> recompute metrics for a registry-served sanity check
...
```bash
cd chap23-mlflow-step-by-step-recap...
```
...
```bash
docker compose ps
# mlflow-recap-23    Up X seconds (healthy)
```
...
- Experiment: `experiment_register_posthoc`
- 1 run + a NEW `elastic-api-2` entry in the **Models** tab, version 1.

> **Chapter quirk.** `mlflow.register_model(...)` blocks until the registry accepts the version. With a slow Postgres or S3 backend you may need `await_creation_for=<seconds>`. Default is 5 minutes -- usually fine, but explicit is better.
...
# Open http://localhost:5000 and inspect the runs in experiment 'experiment_register_posthoc'.
...
3. The MLflow service is healthy: `docker compose ps` -> `mlflow-recap-23 ... healthy`.
...
**Next**: [chap24](../24-practical-work-mlflow-step-by-step-recap-registering-a-model-trained-outside-mlflow-with-cloudpickle.md) -- register a model trained **completely outside MLflow**: load a `.pkl` produced by some other process, then push it into the registry via `mlflow.sklearn.log_model(..., serialization_format="cloudpickle", registered_model_name=...)`.