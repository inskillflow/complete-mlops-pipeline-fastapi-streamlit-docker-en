# chap19 - Step-by-step recap: `mlflow.evaluate` with custom metrics (`make_metric`) and a scatter-plot artifact

The full lesson lives at [../19-practical-work-mlflow-step-by-step-recap-mlflow-evaluate-custom-metrics-and-scatter-artifact.md](../19-practical-work-mlflow-step-by-step-recap-mlflow-evaluate-custom-metrics-and-scatter-artifact.md).

> **In one line.** This chapter is about how to **plug a **custom scalar metric** (built via `mlflow.metrics.make_metric`) and a **custom visual artifact** (a `matplotlib` predictions-vs-targets scatter) into `mlflow.evaluate(...)` to make the auto-evaluator domain-specific**.
...
## What is new vs chap18

- `from mlflow.metrics import make_metric` -> wrap any `def my_fn(eval_df, builtin_metrics): return float(...)` as an MLflow metric
- `extra_metrics=[my_metric]` parameter on `mlflow.evaluate(...)`
- A `def scatter_pred_vs_actual(eval_df, builtin_metrics, artifacts_dir): plt.savefig(...)` returning a dict of paths
- `custom_artifacts=[scatter_pred_vs_actual]` parameter -> the PNG ends up in the run's artifacts/
...
```bash
cd chap19-mlflow-step-by-step-recap...
```
...
```bash
docker compose ps
# mlflow-recap-19    Up X seconds (healthy)
```
...
- Experiment: `experiment_evaluate_custom`
- 1 run with both the **default** metrics and your **custom** scalar metric, plus `pred_vs_actual.png` under Artifacts.

> **Chapter quirk.** Custom-metric callables are passed `(eval_df, builtin_metrics)`: the first is a DataFrame with `prediction` and `target` columns, the second is a dict of the already-computed builtin metrics. Use the second to compose ratios cheaply.
...
# Open http://localhost:5000 and inspect the runs in experiment 'experiment_evaluate_custom'.
...
3. The MLflow service is healthy: `docker compose ps` -> `mlflow-recap-19 ... healthy`.
...
**Next**: [chap20](../20-practical-work-mlflow-step-by-step-recap-validation-thresholds-with-baseline-dummyregressor.md) -- turn the evaluator into a **release gate**: declare `MetricThreshold` objects and a `baseline_model` (a `DummyRegressor`) so `mlflow.evaluate(...)` raises if the candidate model is worse than the baseline. CI/CD-ready ML quality control.