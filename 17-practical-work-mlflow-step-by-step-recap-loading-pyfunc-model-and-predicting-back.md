...
# Chapter 17 — Step-by-step recap: loading a logged pyfunc model with `mlflow.pyfunc.load_model` and predicting again
...
| 2 | [What we add today vs chap 16](#section-2) |
...
Chapter 16 packaged an ElasticNet inside a `pyfunc` artifact. Today we close the loop: we **load** that artifact back with **`mlflow.pyfunc.load_model`** and call its `.predict()` to make sure round-tripping the model preserves its behaviour.
...
## 2. What we add today vs chap 16
...
Everything else (`SklearnWrapper`, `joblib`, `conda_env`, `set_tags`, `autolog(...,log_models=False)`) is identical to chap 16.
...
> [!IMPORTANT]
> Never hardcode a `run_id` in your training script (the snippet in chap 16 had `runs:/ee0c9144.../...` hardcoded — that's a bug waiting to happen as soon as you re-run the script). Always use `model_info.model_uri` or look up the run dynamically with `MlflowClient`.
...
```text
chap17-mlflow-step-by-step-recap-loading-pyfunc-model-and-predicting-back/
├── README.md
├── docker-compose.yml
├── data/
...
└── trainer/
    ├── Dockerfile
    ├── requirements.txt        ← same as 16 (joblib + cloudpickle)
...
Identical to chap 16 (standard SQLite + joblib + cloudpickle in trainer).
...
```bash
cd chap17-mlflow-step-by-step-recap-loading-pyfunc-model-and-predicting-back
...
Next: **[Chapter 18](./18-practical-work-mlflow-step-by-step-recap-mlflow-evaluate-default-regressor.md)** — instead of computing RMSE/MAE/R² by hand after `load_model`, let MLflow do it (and a lot more) with **`mlflow.evaluate(model_uri, eval_df, targets=..., model_type="regressor")`**.
...
<p align="center">
  <strong>End of Chapter 17 — loading a pyfunc model and predicting back</strong><br/>
