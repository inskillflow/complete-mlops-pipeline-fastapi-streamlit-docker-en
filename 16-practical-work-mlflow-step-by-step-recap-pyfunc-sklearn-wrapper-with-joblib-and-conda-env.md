...
# Chapter 16 — Step-by-step recap: `mlflow.pyfunc.log_model` with a custom `SklearnWrapper` (`PythonModel`), `joblib`, and a hand-crafted `conda_env`
...
| 2 | [What we add today vs chap 15](#section-2) |
...
## 2. What we add today vs chap 15
...
```text
chap16-mlflow-step-by-step-recap-pyfunc-sklearn-wrapper-with-joblib-and-conda-env/
├── README.md
├── docker-compose.yml
├── data/
...
Same structure as 15. Only the trainer `requirements.txt` changes (adds `joblib` and `cloudpickle`).
...
```bash
cd chap16-mlflow-step-by-step-recap-pyfunc-sklearn-wrapper-with-joblib-and-conda-env
...
| Chapter | Concept |
|---|---|
| 01 → 03 | Basics, `get_tracking_uri`, full ElasticNet |
| 04 → 06 | Docker trainer, env-var URI, `create_experiment` |
| 07 → 09 | `active_run`, `last_active_run`, `log_artifacts`, `set_tags` |
| 10 → 12 | Multiple runs, multiple experiments, `autolog` |
| 13 | PostgreSQL backend + S3 overview |
| 14 | Model signature (manual `ModelSignature` + automatic `infer_signature`) |
| 15 | Manual signature drill-down (`Schema` + `ColSpec`) |
| **16** | **`pyfunc.log_model` + `SklearnWrapper` + `joblib` + `conda_env`** |

Next: **[Chapter 17](./17-practical-work-mlflow-step-by-step-recap-loading-pyfunc-model-and-predicting-back.md)** — close the loop: **load the pyfunc model back** with `mlflow.pyfunc.load_model(f"runs:/{run_id}/sklearn_mlflow_pyfunc")`, re-run `.predict()` on the test set, and confirm the round-trip produces identical predictions to the original training run.
...
<p align="center">
  <strong>End of Chapter 16 — pyfunc log_model with SklearnWrapper, joblib and conda_env</strong><br/>
