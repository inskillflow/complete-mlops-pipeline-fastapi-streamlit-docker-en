...
# Chapter 09 — Step-by-step recap: attaching metadata to runs with `mlflow.set_tags({...})`
...
| 2 | [What we add today vs chap 08](#section-2) |
...
## 2. What we add today vs chap 08
...
That's the whole change. Everything else (env-var URI, multi-service Docker, bulk `log_params`/`log_metrics`, `log_artifacts("data/")`, `get_artifact_uri`) is **identical to chap 08**.
...
| **Experiment** | `MlflowClient().set_experiment_tag(exp_id, key, value)` *or* the `tags=` argument of `create_experiment` (see chap 06) | The whole experiment |
...
```text
chap09-mlflow-step-by-step-recap-attaching-metadata-with-set-tags/
├── README.md
├── docker-compose.yml
├── data/
...
Same as 08, just a unique `container_name`:
...
services:
  mlflow:
    ...
    container_name: mlflow-recap-09
    ports:
      - "5000:5000"
    volumes:
  ...
  trainer:
    ...
    container_name: trainer-recap-09
    environment:
...
```bash
cd chap09-mlflow-step-by-step-recap-attaching-metadata-with-set-tags
...
3. Tab **Artifacts** → unchanged from chap 08 (model + the 3 CSVs).
...
| **`set_tags` for searchable metadata** | **09** ← you are here |

Next: **[Chapter 10](./10-practical-work-mlflow-step-by-step-recap-multiple-runs-in-one-experiment.md)** — factor the per-run logic into a helper `train_one_run(name, alpha, l1_ratio, ...)` and loop over a list of `CONFIGS` to create **multiple runs in one experiment**, ready for comparison in the UI.

You can stop here and still have a perfectly serviceable MLflow workflow. From here you can also branch into:

- **Chapter 12** — autologging (`mlflow.sklearn.autolog`) — let MLflow log everything by itself.
- **Chapter 14** — model **signature + input example** — describe what your model expects.
- **Chapter 23** — the **Model Registry** post-hoc with `mlflow.register_model` — version, stage, promote.
- **Chapter 26b** — the MLflow **CLI** as a separate Docker service — automate cleanup, exports, and audits.
...
<p align="center">
  <strong>End of Chapter 09 — set_tags for run metadata</strong><br/>
