...
# Chapter 06 — Step-by-step recap: `create_experiment` with tags and a custom `artifact_location`
...
| 2 | [What we add today vs chap 05](#section-2) |
...
## 2. What we add today vs chap 05
...
`MLFLOW_TRACKING_URI` keeps coming from the env var (the chap 05 fix). We never hard-code anything in `train.py`.
...
if __name__ == "__main__":
    ...
    # === Tracking URI from env (chap 05 habit) ===
...
services:
  mlflow:
    ...
    container_name: mlflow-recap-06
    ports:
      - "5000:5000"
    volumes:
      - mlflow-db:/mlflow/database
  ...
  trainer:
    ...
    container_name: trainer-recap-06
    environment:
...
Next: **[Chapter 07](./07-practical-work-mlflow-step-by-step-recap-active-run-and-last-active-run-with-start-end-run.md)** — instrument the script with `mlflow.active_run()` / `mlflow.last_active_run()` and the imperative `start_run()` / `end_run()` pair, so you can inspect the live run object and refer to the previous run after it has closed.
...
<p align="center">
  <strong>End of Chapter 06 — `create_experiment` with tags and a custom `artifact_location`</strong><br/>
