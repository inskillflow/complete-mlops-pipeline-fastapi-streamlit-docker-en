# chap15 - Step-by-step recap: manual model signature with `Schema` + `ColSpec` + `ModelSignature`

The full lesson lives at [../15-practical-work-mlflow-step-by-step-recap-manual-model-signature-schema-colspec.md](../15-practical-work-mlflow-step-by-step-recap-manual-model-signature-schema-colspec.md).

> **In one line.** This chapter is about how to **hand-craft the model signature from primitives (`ColSpec`, `Schema`, `ModelSignature`) so you control every input/output type explicitly -- mandatory whenever `infer_signature` guesses wrong**.
...
## What is new vs chap14

- `from mlflow.types import Schema, ColSpec` and `from mlflow.models import ModelSignature`
- `input_schema = Schema([ColSpec("double", "alcohol"), ColSpec("double", "pH"), ...])`
- `output_schema = Schema([ColSpec("long", "quality")])` (integer target!)
- `mlflow.sklearn.log_model(lr, "model", signature=ModelSignature(inputs=input_schema, outputs=output_schema), input_example=...)`
...
```bash
cd chap15-mlflow-step-by-step-recap...
```
...
```bash
docker compose ps
# mlflow-recap-15    Up X seconds (healthy)
```
...
```bash
docker compose run --rm trainer --alpha 0.4 --l1_ratio 0.6
```
...
- Experiment: `experiment_manual_signature`
- 1 run with an explicit `signature:` block in `MLmodel`. `output` is typed as `long` (integer) -- exactly the contract `mlflow models serve` will enforce.

> **Chapter quirk.** `ColSpec` types are the MLflow-DataType vocabulary: `boolean`, `integer`, `long`, `float`, `double`, `string`, `binary`, `datetime`. Not the pandas names. Use `long` for int64 columns, not `integer`.
...
# Open http://localhost:5000 and inspect the runs in experiment 'experiment_manual_signature'.
...
3. The MLflow service is healthy: `docker compose ps` -> `mlflow-recap-15 ... healthy`.
...
**Next**: [chap16](../16-practical-work-mlflow-step-by-step-recap-pyfunc-sklearn-wrapper-with-joblib-and-conda-env.md) -- wrap the ElasticNet in a `PythonModel` (`SklearnWrapper`) and log it with `mlflow.pyfunc.log_model`, bundling `joblib`, a `conda_env` dict and a `code_path` into one portable artifact.