...
# Chapter 15 — Step-by-step recap: writing a model signature **by hand** with `Schema`, `ColSpec` and `ModelSignature`
...
| 2 | [What we add today vs chap 14](#section-2) |
...
Chapter 14 introduced `infer_signature` as the easy path (one line, reads the real data). Today we drill into the **manual path** — writing the schema from scratch using `Schema`, `ColSpec` and `ModelSignature`. This matters when:
...
## 2. What we add today vs chap 14
...
| Experiment name: `"experiment_signature"` | Same experiment name as chap 14 — the two runs appear side-by-side for comparison. |

Everything else (env-var URI, `autolog(log_models=False,...)`, `set_tags`, `log_artifact`) is identical to chap 14.
...
| Format | Example |
|---|---|
| **Pandas DataFrame** | `test_x.head(5)` (what chap 14 used) |
...
```text
chap15-mlflow-step-by-step-recap-manual-model-signature-schema-colspec/
├── README.md
├── docker-compose.yml
├── data/
...
All identical to chap 14 (standard SQLite setup).
...
```bash
cd chap15-mlflow-step-by-step-recap-manual-model-signature-schema-colspec
docker compose up -d --build mlflow
docker compose run --rm trainer --alpha 0.7 --l1_ratio 0.7
```

In the UI ([http://localhost:5000](http://localhost:5000)) → **`experiment_signature`** → you now have **two runs**: one from chap 14 (`infer_signature`) and one from today (manual schema).
...
- `signature.json`:
  ...
  ```
  Note **`"long"`** for the output — the chap 14 run had `"double"` (inferred from the float array). Same model, different declared output type.
...
| | chap 14 (`infer_signature`) | chap 15 (manual) |
|---|---|---|
...
Next: **[Chapter 16](./16-practical-work-mlflow-step-by-step-recap-pyfunc-sklearn-wrapper-with-joblib-and-conda-env.md)** — wrap the ElasticNet in a **`PythonModel`** (`SklearnWrapper`) and log it with `mlflow.pyfunc.log_model`, bundling `joblib`, a `conda_env` dict, and a `code_path` into a single portable artifact.
...
<p align="center">
  <strong>End of Chapter 15 — manual model signature with Schema + ColSpec + ModelSignature</strong><br/>
