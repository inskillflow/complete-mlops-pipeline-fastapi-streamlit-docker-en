...
# Chapter 14 — Step-by-step recap: model **signature** — manual (`ModelSignature` / `Schema` / `ColSpec`) vs automatic (`infer_signature`)
...
| 2 | [What we add today vs chap 13](#section-2) |
...
## 2. What we add today vs chap 13
...
```text
chap14-mlflow-step-by-step-recap-model-signature-manual-and-infer-signature/
├── README.md
...
Back to SQLite (standard setup from 07→12):
...
services:
  mlflow:
    ...
    container_name: mlflow-recap-14
    ports:
      - "5000:5000"
    volumes:
  ...
  trainer:
    ...
    container_name: trainer-recap-14
    environment:
...
Identical to chap 07→12 (standard SQLite image, no psycopg2).
...
```bash
cd chap14-mlflow-step-by-step-recap-model-signature-manual-and-infer-signature
...
| 04 → 06 | Dockerised trainer, env-var URI, `create_experiment` |
...
| **14** | **Model signature (manual + infer)** |

Next: **[Chapter 15](./15-practical-work-mlflow-step-by-step-recap-manual-model-signature-schema-colspec.md)** — drill deeper into the **manual** signature path: build `Schema` + `ColSpec` + `ModelSignature` explicitly so you understand the exact types and shapes MLflow encodes, including how to declare integer-typed targets vs continuous features.
...
<p align="center">
  <strong>End of Chapter 14 — model signature (manual + infer_signature)</strong><br/>
