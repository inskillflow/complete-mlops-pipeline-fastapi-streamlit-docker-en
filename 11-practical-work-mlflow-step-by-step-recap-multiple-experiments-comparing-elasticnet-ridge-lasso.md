...
# Chapter 11 — Step-by-step recap: launching **multiple experiments** (ElasticNet vs Ridge vs Lasso)
...
| 2 | [What we add today vs chap 10](#section-2) |
...
chap 10 compared **hyperparameters** of one algorithm (ElasticNet) by putting many runs in a single experiment.
...
## 2. What we add today vs chap 10
...
Everything else (env-var URI, multi-service Docker, `set_tags`, `log_artifacts("data/")`, `last_active_run`) is identical to chap 10.
...
```text
chap11-mlflow-step-by-step-recap-multiple-experiments-elasticnet-ridge-lasso/
├── README.md
├── docker-compose.yml
├── data/
...
Same skeleton as 10, only `container_name`s change:
...
services:
  mlflow:
    ...
    container_name: mlflow-recap-11
    ports:
      - "5000:5000"
    volumes:
  ...
  trainer:
    ...
    container_name: trainer-recap-11
    environment:
...
```bash
cd chap11-mlflow-step-by-step-recap-multiple-experiments-elasticnet-ridge-lasso
...
Next: **[Chapter 12](./12-practical-work-mlflow-step-by-step-recap-automating-logging-with-mlflow-autolog.md)** — replace half of `train_one_run` with a single line: **`mlflow.autolog()`**. MLflow logs your params, metrics, model, signature and input example **for you**, just by detecting the `.fit()` call.
...
<p align="center">
  <strong>End of Chapter 11 — multiple experiments (ElasticNet vs Ridge vs Lasso)</strong><br/>
