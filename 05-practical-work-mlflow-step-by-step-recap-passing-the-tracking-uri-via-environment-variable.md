...
# Chapter 05 — Step-by-step recap: passing the MLflow tracking URI via an environment variable
...
Fix the bug we left at the end of Chapter 04: runs were silently logged to a dead container's filesystem. Today we wire the trainer to the server **without** hard-coding any URL in `train.py`. The trick is the `MLFLOW_TRACKING_URI` environment variable, set in `docker-compose.yml`.
...
Same files as chap 04, two small diffs.
...
Diff vs chap 04 (only the lines marked `# NEW`):
...
Diff vs chap 04 (only the `environment:` block on the trainer):
...
services:
  mlflow:
    ...
    container_name: mlflow-recap-05
    ports:
      - "5000:5000"
    volumes:
  ...
  trainer:
    ...
    container_name: trainer-recap-05
    environment:                                              # NEW
...
<p align="center">
  <strong>End of Chapter 05 — Passing the tracking URI via an env var</strong><br/>
