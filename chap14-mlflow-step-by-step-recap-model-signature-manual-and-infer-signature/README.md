# chap14 - Step-by-step recap: model signature (manual `ModelSignature` + automatic `infer_signature`)

The full lesson lives at [../14-practical-work-mlflow-step-by-step-recap-model-signature-manual-and-infer-signature.md](../14-practical-work-mlflow-step-by-step-recap-model-signature-manual-and-infer-signature.md).

> **In one line.** This chapter is about how to **attach a typed contract to the logged model so `mlflow models serve` can validate inbound JSON requests, via two equivalent paths: the explicit `ModelSignature` constructor and the convenience `infer_signature(test_x, preds)` helper**.
...
## What is new vs chap13

- `signature = infer_signature(test_x, preds)` -> one-liner, types inferred from a numpy/pandas sample
- `mlflow.sklearn.log_model(lr, "model", signature=signature, input_example=test_x.head(2))`
- Side-by-side with the manual style: `ModelSignature(inputs=Schema([...]), outputs=Schema([...]))` (chap15 drill-down)
- Open the run -> `Artifacts/model/MLmodel` -> see the `signature:` block embedded
...
```bash
cd chap14-mlflow-step-by-step-recap...
```
...
```bash
docker compose ps
# mlflow-recap-14    Up X seconds (healthy)
```
...
- 1 run; open `Artifacts/model/MLmodel` and confirm the `signature:` block is present + `input_example.json` is alongside.

> **Chapter quirk.** `infer_signature` reads the numpy dtype, NOT your DataFrame schema. If you load a CSV with mixed-typed columns, cast explicitly (`df.astype({...})`) BEFORE calling `infer_signature`, otherwise you get `object` columns that `mlflow models serve` won't enjoy.
...
# Open http://localhost:5000 and inspect the runs in experiment 'experiment_signature'.
...
3. The MLflow service is healthy: `docker compose ps` -> `mlflow-recap-14 ... healthy`.
...
**Next**: [chap15](../15-practical-work-mlflow-step-by-step-recap-manual-model-signature-schema-colspec.md) -- drill into the **manual** signature path with `Schema` + `ColSpec` + `ModelSignature` so you can hand-craft the contract exactly (useful when the auto-inferred dtype differs from your production schema).